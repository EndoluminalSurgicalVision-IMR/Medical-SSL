...
class RKBTrainer(BaseTrainer):
    def __init__(self, config):
        super(RKBTrainer, self).__init__(config)
        assert config.model == 'Simple'
        self.bce_criterion = nn.BCELoss(reduction='mean')
        self.bce_criterion.to(self.device)

    def set_input(self, sample):
        input, order_label, hor_rot_label, ver_rot_label = sample
        ...
        # self.hor_rot_label = self.hor_rot_label.view(-1, 1)
        # self.ver_rot_label = self.ver_rot_label.view(-1, 1)
        # [B, N, 1, 128, 128, 32], [B] [B, N]
        # print('size', self.input.size(), self.order_label.size(), self.hor_rot_label.size(), self.ver_rot_label.size())

    def forward(self):
        self.order_pred, self.hor_rot_pred, self.ver_rot_pred = self.model(self.input)
    ...
    def backward(self):
        ...
        self.loss = (self.order_loss + self.rot_loss) / 2
        self.loss.backward()
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.epochs):
            ...
            self.recorder.logger.info("Epoch {}, order acc is {:4f}, hor rot acc is {:.4f},"
                                      " ver rot acc is {:.4f}".format(epoch + 1, order_valid_acc,
                                                                      hor_rot_valid_acc,
                                                                      ver_rot_valid_acc))
            valid_acc = (order_valid_acc + (hor_rot_valid_acc + ver_rot_valid_acc) / 2) / 2
