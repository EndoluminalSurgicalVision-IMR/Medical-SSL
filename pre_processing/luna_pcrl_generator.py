...
# Adapted from https://github.com/Luchixiang/PCRL/

"""

For PCRL pretext task;
Luna dataset:
...
parser.add_option("--data", dest="data", help="the directory of LUNA16 dataset", default='../../Data/LUNA2016',
                  type="string")
parser.add_option("--save", dest="save", help="the directory of processed 3D cubes",
                  default='../data/PCRL_Cubes', type="string")
...
col_size = [(64, 64, 32), (96, 96, 48), (96, 96, 96), (32, 32, 16), (112, 112, 64)]
...
def infinite_generator_from_one_volume(img_array, save_dir, name):
    ...
    num_pair = 0
    while True:
        crop_window1, crop_window2 = crop_pair(img_array)
        crop_window = np.stack((crop_window1, crop_window2), axis=0)
        # print(crop_window.shape)
        np.save(os.path.join(save_dir, name + '_img_' + str(num_pair) + '.npy'), crop_window)
        num_pair += 1
    ...
    while True:
        ...
        while True:
            ...
            iou = cal_iou(
                ...
                (start_x2, start_x2 + crop_rows2, start_y2, start_y2 + crop_cols2, start_z2,
                 start_z2 + crop_deps2))
        ...
        if np.sum(d_img1) > config.lung_max * crop_deps1 * crop_rows1 * crop_cols1:
            continue
        # print(np.sum(d_img1))
        if np.sum(d_img2) > config.lung_max * crop_deps2 * crop_rows2 * crop_cols2:
            continue
        return crop_window1[:, :, :input_depth], crop_window2[:, :, :input_depth]


def get_self_learning_data(fold):
    save_path = '../data/PCRL_Cubes'
